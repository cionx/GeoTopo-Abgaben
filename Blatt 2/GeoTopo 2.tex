...
\begin{center}
 \begin{tikzpicture}[node distance=5em,auto]
  \node (Sn) {$S^n$};
  \node (Rn+1) [right of = Sn, node distance = 9em] {$\R^{n+1}-\{0\}$};
  \node (Sn/) [below of = Sn] {$S^n/\sim$};
  \node (Rn+1/) [below of = Rn+1] {$(\R^{n+1}-\{0\})/\sim$};
  \draw[->] (Sn) to node {$\iota$} (Rn+1);
  \draw[->] (Sn/) to node [swap] {$\iota'$} (Rn+1/);
  \draw[->] (Sn) to node [swap] {$\pi_1$} (Sn/);
  \draw[->] (Rn+1) to node {$\pi_2$} (Rn+1/);
 \end{tikzpicture}
\end{center}
...
\begin{center}
 \begin{tikzpicture}[node distance=5em,auto]
  \node (Dn) {$\tilde{D}^n$};
  \node (Sn) [right of = Dn, node distance = 7em] {$S^n$};
  \node (Dn/) [below of = Dn] {$\tilde{D}^n/\sim$};
  \node (Sn/) [below of = Sn] {$S^n/\sim$};
  \draw[->] (Dn) to node {$\iota$} (Sn);
  \draw[->] (Dn/) to node [swap] {$\iota'$} (Sn/);
  \draw[->] (Dn) to node [swap] {$\pi_1$} (Dn/);
  \draw[->] (Sn) to node {$\pi_2$} (Sn/);
 \end{tikzpicture}
\end{center}

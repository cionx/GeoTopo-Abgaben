...
\begin{figure}[ht]\centering
 \begin{tikzpicture}[auto, node distance = 6em]
  ...
  \draw[->] (X x I) to node[swap] {$\pi$} (Tf);
  \draw[->] (X x R) to node {$p$} (X x R / Z);
 \end{tikzpicture}
...
\begin{figure}\centering
 \begin{tikzpicture}[auto, node distance = 6em]
  \node (X x R) {$X \times \R$};
  \node (I) [right of = X x R, node distance = 8em] {$[t,t-1]$};
  \node (Tf) [below of = X x R] {$T_f$};
  \node (S1) [below of = I] {$S^1$};
  \draw[->] (I) to node[swap] {$\tilde{\gamma}$} (X x R);
  \draw[->] (X x R) to node[swap] {$p$} (Tf);
  \draw[->] (I) to node {$\exp_{|[t-1,t]}$} (S1);
  \draw[->] (S1) to node {$s$} (Tf);
 \end{tikzpicture}
 \begin{tikzpicture}[auto, node distance = 6em]
  \node (X x R) {$X \times \R$};
  \node (I) [right of = X x R, node distance = 8em] {$[t,t-1]$};
